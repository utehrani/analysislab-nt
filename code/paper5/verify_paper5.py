...
#   4. B-decomposition: B = Σ_p (log p)² · Re(Z_p)
...
print("\n[4] Curvature-bias: B < 0 and B = Σ_p (log p)² Re(Z_p)")
...
check(f"B_prop = Σ_p(log p)²Re(Z_p) < 0  (B_prop={B_prop:.1f})",
      B_prop < 0, f"B_prop={B_prop:.4f}")
print(f"    B (curvature) = {B_direct:.4f}  [includes γ_k² weights]")
print(f"    B_prop        = {B_prop:.4f}  [Proposition 5.3 object]")
