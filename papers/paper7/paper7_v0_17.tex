...
% Date:     June 2026
% Version:  v0.17
% Zenodo:   https://doi.org/10.5281/zenodo.20440671
% License:  CC BY 4.0
% ============================================================
...
\author{Ulrich Tehrani}
\date{Research Note \textperiodcentered\ June~2026
      \textperiodcentered\ v0.17}
...
The trivial bound $\mathcal{W}_{\varepsilon,N}\cdot P(\kappa)\approx 1277$
exceeds $|O'(\tfrac12)|$ by approximately $516\times$
($|O'(\tfrac12)|/(\mathcal{W}_{\varepsilon,N}\cdot P(\kappa))\approx 0.00194$).
Separately, the internal three-term balance satisfies
...
\[
S_k(\kappa):=\sum_{p\le\kappa}(\log p)\sin(\gamma_k\log p).
\]
\begin{remark}
The symbol $S_k(\kappa)$ is distinct from $M_k(\kappa)$ of
Paper~4~\cite{Paper4}, which denotes the partial-sum maximum
$\max_{y\le\kappa}|\sum_{p\le y}\sin(\gamma_k\log p)|$.
Both symbols appear in the series; the present paper uses
$S_k(\kappa)$ to avoid ambiguity.
\end{remark}
...
$O'(\tfrac12)=+2.4751$; $O''(\tfrac12)=+38\,685.1$; $B=-19\,342.5$;
$\varepsilon_{\mathrm{int}}(53)>0.200$;
the trivial bound exceeds $|O'(\tfrac12)|$ by approximately $516\times$,
while the internal three-term balance gives
$|T_{\mathrm{pol}}+T_{\mathrm{main}}|/|O'(\tfrac12)|\approx 15.7$.
...
$S_k(\kappa)=\sum_{p\le\kappa}(\log p)\sin(\gamma_k\log p)$.
The following hypothesis asserts a logarithmic saving in $S_k(\kappa)$
...
The hypothesis $|S_k(\kappa)|\le C\cdot P(\kappa)/(\log\gamma_k)^A$
...
Writing $S_k(\kappa)=\mathrm{Im}\,\sum_{p\le\kappa}(\log p)\,p^{i\gamma_k}$,
...
the bound on $|S_k(\kappa)|$ with the $P$-normalisation has a larger right-hand side.
The $P$-form is nonetheless the numerically consistent choice:
$|S_k(\kappa)|/P(\kappa)$ stays bounded near $\pm 1/\gamma_k$
[numerical], while $|S_k(\kappa)|/\pi(\kappa)$ grows logarithmically [numerical].
Numerically, $|S_k(\kappa)|/P(\kappa)$ oscillates near $\pm 1/\gamma_k$
...
imply the logarithmic saving in $S_k(\kappa)$ required by
Assumption~\Astar.
...
\begin{proof}
By Assumption~\Astar, $|S_k(\kappa)|\le C_0\,P(\kappa)/(\log\gamma_k)^A$
for every $k\ge 1$.
Since $O'(\tfrac12)=-\sum_{k=1}^N e^{-\varepsilon^2\gamma_k^2}\gamma_k\,S_k(\kappa)$,
we obtain
\[
|O'(\tfrac12)|
...
saving in $|S_k(\kappa)|/P(\kappa)$ asserted by Assumption~\Astar?
\end{openproblem}
...
\small\emph{Paper~7 v0.17 \textperiodcentered\ June~2026
